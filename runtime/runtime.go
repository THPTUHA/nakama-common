...
Here's the smallest example of a Go module written with the server runtime.
	...
	import (
		...
		"github.com/THPTUHA/nakama-common/runtime"
	)
...
To setup your own project to build modules for the game server you can follow these steps.
 ...
 1. Build Nakama from source:
    go get -d github.com/THPTUHA/nakama-common
    cd $GOPATH/src/github.com/THPTUHA/nakama-common
...
NOTE: It is not possible to build plugins on Windows with the native compiler toolchain but they can be cross-compiled and run with Docker.
 ...
 4. Start Nakama with your module:
    $GOPATH/src/github.com/THPTUHA/nakama-common/nakama --runtime.path $GOPATH/src/plugin_project/modules
...
import (
	...
	"github.com/THPTUHA/nakama-common/api"
	"github.com/THPTUHA/nakama-common/rtapi"
...
type NakamaModule interface {
	...
	UsersGetCustomId(ctx context.Context, customIDs []string) ([]*api.User, error)
	...
	NotificationsMarkRead(ctx context.Context, userID string, ids []string) error
